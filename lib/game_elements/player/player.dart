...
import 'skins/skin1/player_skin_1.dart';
...
class Player extends StatefulTileGameObject {
  ...
  int _getMovementCount(int current, int target, int tileCount, bool circular) {
    ...
    int getCurcularTiles() {
      ...
      final looped =
          (level.tiles.length - (_targetTile - level.activeTile).abs());
  ...
  void _updatePosition(DateTime frameTimestamp) {
    switch (lifecycleState) {
      case PlayerLive _ || PlayerFlyThroughLevel _:
        {
      ...
      case PlayerFlyFromLevel _ || PlayerFlyToLevel _:
        {
          pivot.updatePosition(
              offset:
                  (lifecycleState as PlayerFlyOutsideLevel).currentPosition);
        }
      default:
        throw UnimplementedError(
            "Unhandled player lifecycle state, ${lifecycleState.runtimeType}");
    }
  }
  ...
  set setTargetTile(int value) {
    ...
    _movementCount = _getMovementCount(
        pivot.tileNumber, _targetTile, level.tiles.length, level.circlular);
  }

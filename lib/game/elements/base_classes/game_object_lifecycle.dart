...
import 'package:chromatic_chasm/game/helpers/transition_functions.dart';
...
mixin class TransitionLifeCycle implements TimefulLifecycle {
  ...
  void configureTransition(
    Positionable startOffsetPivot,
    Positionable endOffsetPivot, {
    Duration? duration,
  ...
  Positionable get currentPosition {
    assert(
      _startOffsetPivot != Positionable.zero() ||
          _endOffsetPivot != Positionable.zero(),
      "Animation has not been configured",
    );
    _pivot = _transitionFunction(
      _easingFunction(timeFraction),
      _startOffsetPivot,
      _endOffsetPivot,
...
mixin class FlyingLifecycle implements GameObjectLifecycle {
  ...
  Positionable getCurrentPosition(DateTime frameTimestamp) {
    ...
    final double delta =
        frameTimestamp.difference(lastFrame).inMilliseconds / Drawable.syncTime;
...
final class PlayerFlyThroughLevel extends PlayerLifecycle
    with TimefulLifecycle {

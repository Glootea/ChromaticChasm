...
class Positionable extends Vector3 implements Transformable {
  ...
  @override
  Positionable operator +(covariant other) =>
      Positionable.zero()..setValues(x + other.x, y + other.y, z + other.z);
  @override
  Positionable operator -(covariant other) =>
      Positionable.zero()..setValues(x - other.x, y - other.y, z - other.z);
  @override
  Positionable operator *(covariant scale) =>
      Positionable.zero()..setValues(x * scale, y * scale, z * scale);
  @override
  Positionable operator /(covariant scale) =>
      Positionable.zero()..setValues(x / scale, y / scale, z / scale);
...
extension PositionFunctions on Positionable {
  static Positionable median(Positionable first, Positionable second) =>
      (first + second) * 0.5;
...
class TilePositionable extends Positionable {
  ...
  Positionable get globalPosition {
    ...
    return PositionFunctions.positionWithFraction(
          leftLinePoint,
          rightLinePoint,
          widthFraction,
        ) +
